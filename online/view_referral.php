...
// --- FETCH ALL MEMBERS AS TREE (for deep levels) ---
$allMembers = [];
$tree = [];

// Load into array
$all = $mysqli->query("SELECT mlm_id, ref_id FROM members");
while($m = $all->fetch_assoc()) {
    $allMembers[$m['mlm_id']] = $m['ref_id'];
    $tree[$m['ref_id']][] = $m['mlm_id'];
}

// Recursive function to count full downline
function getTotalDownline($id, $tree) {
    if (!isset($tree[$id])) return 0;
    $count = count($tree[$id]);
    foreach ($tree[$id] as $child) {
        $count += getTotalDownline($child, $tree);
    }
    return $count;
}
            ...
            <table class="table table-hover align-middle text-center">
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    ...
                    <tr>
                        ...
                        <td><?= getTotalDownline($row['mlm_id'], $tree) ?></td>                 
                        <td><?= htmlspecialchars($row['phone']) ?></td>
                        <td><?= htmlspecialchars($row['status']) ?></td>
                       <?php 
$referral_link = "https://www.dujeeeduzonenepal.org/registration_ref.php?ref_id=" . urlencode($row['mlm_id']); 
?>
<td>
